...
clear;
%----------------------------problem 2-------------------------------------
A_c = [0 0 1 0;0 0 0 1;-2 1 -1.6 0.8;1 -1 0.8 -0.8];
B_c = [0 0;0 0;1 0;0 1];
C_c = [0 1 0 0;0 0 0 1];
dT = 0.1;
A_d = expm(A_c * dT);
B_d = (expm(A_c * dT) - eye(4))*inv(A_c)*B_c;
sigma_u = [0.2 0;0 0.2];
sigma_v = [0.1 0;0 0.1];
...
while(n2<100)
x2_k = x2_k1;
urand = normrnd(0,0.2*eye(2));
uk=[urand(1,1);urand(2,2)];
x2_k1 = A_d * x2_k + B_d*uk;
xp2(:,n2+1) = x2_k1;
n2=n2+1;
end
t=0:0.1:10-0.1;
h1 = plot(t, xp(1,:), '-', t, xp(2,:),'-.', t, xp2(1,:), '--', t, xp2(2,:),':');
%h1 = plot(t, xp(1,:), '-', t, xp(2,:),'-.');
xlabel('Time(sec)');
ylabel('masses position');
legend('mass 1','mass 2','mass 1 u~(0,0.2I)','mass 2 u~(0,0.2I)');
...
figure
h2 = plot(t0,zz(1,:),'-',t0,zz(2,:),'-.',t0,zz2(1,:),'--',t0,zz2(2,:),':' );
